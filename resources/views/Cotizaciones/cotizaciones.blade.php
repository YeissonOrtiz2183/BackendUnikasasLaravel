<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Registro de cotizaciones</title>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-ka7Sk0Gln4gmtz2MlQnikT1wXgYsOg+OMhuP+IlRH9sENBO0LRn5q+8nbTov4+1p" crossorigin="anonymous"></script>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
    <link rel="icon" href="data:image/png;base64,   ">
</head>
<body>
  <form action="">
  <a class="btn btn-" onclick="history.back()" role="button"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-house-door-fill" viewBox="0 0 16 16">
    <path d="M6.5 14.5v-3.505c0-.245.25-.495.5-.495h2c.25 0 .5.25.5.5v3.5a.5.5 0 0 0 .5.5h4a.5.5 0 0 0 .5-.5v-7a.5.5 0 0 0-.146-.354L13 5.793V2.5a.5.5 0 0 0-.5-.5h-1a.5.5 0 0 0-.5.5v1.293L8.354 1.146a.5.5 0 0 0-.708 0l-6 6A.5.5 0 0 0 1.5 7.5v7a.5.5 0 0 0 .5.5h4a.5.5 0 0 0 .5-.5z"/>
  </svg></a>

  <h1 style="text-align: center; margin-bottom: 1%">Registro de cotizaciones</h1>
  <div class="container">
    <div class="row row-cols-5">
      {{-- <div class="col- "><div class="dropdown">Codigo
        <a class="btn btn-secondary dropdown-toggle" href="#" role="button" id="dropdownMenuLink" data-bs-toggle="dropdown" aria-expanded="false">
          Filtrar
        </a>

        <ul class="dropdown-menu" aria-labelledby="dropdownMenuLink">
          <li><a class="dropdown-item" href="#">Codigo</a></li>
        </ul>
      </div></div> --}}
      {{-- <div class="col- "><div class="dropdown">Cliente
        <a class="btn btn-secondary dropdown-toggle" href="#" role="button" id="dropdownMenuLink" data-bs-toggle="dropdown" aria-expanded="false">
         Filtrar
        </a> --}}

        {{-- <ul class="dropdown-menu" aria-labelledby="dropdownMenuLink">
          <li><a class="dropdown-item" href="#">Cliente</a></li>
        </ul>
      </div></div> --}}

      {{-- <ul class="dropdown-menu" aria-labelledby="dropdownMenuLink">
        <li><select name="" id="">
          @foreach ($cotizaciones as $cotizacion)
            <a class="dropdown-item" href="#">
              <option value="{{ $cotizacion->nombres_cotizante }}">{{ $cotizacion->nombres_cotizante}} {{ $cotizacion->apellidos_cotizante}}</option></a></li>
            @endforeach
            </select>
      </ul>
    </div></div> --}}

    <div class="mb-2 row row row col-2">
      <label for="ciudad" class="col-sm-4 control-label" style="margin-right: 5.5%">Codigo:</label>

      <div class="col-sm-7">
              <select class="form-control text-white bg-dark" class="dropdown" name="codigo" id="codigo">
                <option value="null" selected disabled hidden>Filtrar</option>
                  @foreach ($cotizaciones as $cotizacion)
                      <option value="{{ $cotizacion->id }}">{{ $cotizacion->id}}</option>
                  @endforeach
              </select>
          </div>
  </div>

    <div class="mb-2 row row row col-3">
      <label for="ciudad" class="col-sm-3 control-label">Cliente:</label>

      <div class="col-sm-8">
              <select class="form-control text-white bg-dark" name="cliente" id="cliente">
                <option value="null" selected disabled hidden>Filtrar</option>
                  @foreach ($cotizaciones as $cotizacion)
                      <option value="{{ $cotizacion->nombres_cotizante }}">{{ $cotizacion->nombres_cotizante}} {{ $cotizacion->apellidos_cotizante}}</option>
                  @endforeach
              </select>
          </div>
  </div>

  <div class="mb-2 row row row col-3">
    <label for="ciudad" class="col-sm-3 control-label" class="dropdown">Fecha:</label>
    <div class="col-sm-8">
            <input type="date"class="form-control text-white bg-dark" name="fecha" id="fecha1" style="color-scheme: dark;">
        </div>
</div>

  <div class="mb-2 row row row col-3">
    <label for="ciudad" class="col-sm-3 control-label">Estado:</label>

    <div class="col-sm-8">
            <select class="form-control text-white bg-dark" name="estado" id="estado">
              <option value="null" selected disabled hidden>Filtrar</option>
              <option value="Activa">Activa</option>
              <option value="Cancelada">Cancelada</option>
            </select>
        </div>
</div>

      {{-- <div class="col"><div class="dropdown">Fecha
        <a class="btn btn-secondary dropdown-toggle" href="#" role="button" id="dropdownMenuLink" data-bs-toggle="dropdown" aria-expanded="false">
         Filtrar
        </a>

        <ul class="dropdown-menu" aria-labelledby="dropdownMenuLink">
          <li><a class="dropdown-item" href="#">Fecha</a></li>
        </ul>
      </div></div>
      <div class="col"><div class="dropdown">Estado
        <a class="btn btn-secondary dropdown-toggle" href="#" role="button" id="dropdownMenuLink" data-bs-toggle="dropdown" aria-expanded="false">
          Filtrar
        </a>

        <ul class="dropdown-menu" aria-labelledby="dropdownMenuLink">
          <li><a class="dropdown-item" href="#">Estado</a></li>
        </ul>
      </div></div> --}}
      <input type="submit" class="btn btn-warning" value="Buscar" style="width: 12%">
        {{-- <span class="position-absolute translate-middle  border border-light rounded-circle">
          <span class="visually-hidden"></span>
        </span> --}}

    </div>
    </div>
  </div>
  <br>
    <ul class="dropdown-menu" aria-labelledby="dropdownMenuLink">
      <li><a class="dropdown-item" href="#">Action</a></li>
      <li><a class="dropdown-item" href="#">Another action</a></li>
      <li><a class="dropdown-item" href="#">Something else here</a></li>
    </ul>
  </div>

  </div>
    <div class="container">
        <div class="row row-cols-5">
          <div class="col- border border-dark bg-warning" style="text-align: center"><b>Codigo</b></div>
          <div class="col- border border-dark bg-warning" style="text-align: center"><b>Cliente</b></div>
          <div class="col- border border-dark bg-warning" style="text-align: center"><b>Fecha</b></div>
          <div class="col- border border-dark bg-warning" style="text-align: center"><b>Estado</b></div>
          <div class="col- border border-dark bg-warning" style="text-align: center"><b>Opciones</b>

        </form>

          <!-- Modal -->
          <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title" id="exampleModalLabel">Rechazar Cotizacion</h5>
                  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                  <div class="alert alert-danger" role="alert">
                    Esta seguro de que desea rechazar la cotizacion
                  </div>
                </div>
                <div class="modal-footer">
                  <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">No</button>
                  <button type="button" class="btn btn-primary">Si</button>
                </div>
              </div>
            </div>
          </div></b>
          {{-- <button type="button" class="btn btn-light" data-bs-toggle="modal" data-bs-target="#exampleModal"></button> --}}

          <!-- Modal -->
          {{-- <div class="modal fade" id="exampleModal2" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="D" id="exampleModalLabel"> Eliminar la cotizacion</h5>
                  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                  <div class="alert alert-danger" role="alert">
                  Esta seguro de que desea eliminar la cotizacion
                  </div>
                </div>
                <div class="modal-footer">
                  <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">No</button>
                  <input type="button" class="btn btn-primary">Si</button>
                </div>
              </div>
            </div>
          </div>
          <button type="button" class="btn btn-light" data-bs-toggle="modal" data-bs-target="#exampleModal">

          </button> --}}

          <!-- Modal -->
          <div class="modal fade" id="exampleModal3" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="D" id="exampleModalLabel"> Rechazar la cotizacion</h5>
                  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                  Esta seguro de que desea rechazar la cotizacion
                </div>
                <div class="modal-footer">
                  <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">No</button>
                  <button type="button" class="btn btn-primary">Si</button>
                </div>
              </div>
            </div>
          </div>
          {{-- <button type="button" class="btn btn-light data-bs-toggle="modal" data-bs-target="#exampleModal"></button> --}}

          <!-- Modal -->
          <div class="modal fade" id="exampleModal4" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="D" id="exampleModalLabel">Actualizar</h5>
                  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                  <div class="alert alert-danger" role="alert">
                    <checkbox>Roja </checkbox>
                  </div>
                  <div class="alert alert-warning" role="alert">
                     <checkbox>Naranja</checkbox>
                  </div>
                  <div class="alert alert-info" role="alert">
                     <checkbox>Verde</checkbox>
                  </div>
                </div>
                <div class="modal-footer">
                  <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">No</button>
                  <button type="button" class="btn btn-light">Si</button>
                </div>
              </div>
            </div>
          </div></div>

           <!-- Rows and Columns -->
           @foreach ($cotizaciones as $cotizacion)

           <div class="col- border" style="border: none">{{ $cotizacion->id }}</div>
           <div class="col- border">{{ $cotizacion->nombres_cotizante }} {{ $cotizacion->apellidos_cotizante }}</div>
           <div class="col- border">{{ date('d/m/Y', strtotime($cotizacion->fecha_cotizacion)) }}</div>
           <div class="col- border">{{ $cotizacion->estado_cotizacion }}</div>

           <div class="col- border"><b><a class="btn btn-light" href="{{ url('cotizaciones/create') }}" role="button"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
             <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
             <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z"/>
           </svg></a><a class="btn btn-light" href="{{ url('cotizaciones/'.$cotizacion->id.'/edit') }}" role="button"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" class="bi bi-pencil-fill" viewBox="0 0 16 16">
             <path d="M12.854.146a.5.5 0 0 0-.707 0L10.5 1.793 14.207 5.5l1.647-1.646a.5.5 0 0 0 0-.708l-3-3zm.646 6.061L9.793 2.5 3.293 9H3.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.207l6.5-6.5zm-7.468 7.468A.5.5 0 0 1 6 13.5V13h-.5a.5.5 0 0 1-.5-.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.5-.5V10h-.5a.499.499 0 0 1-.175-.032l-.179.178a.5.5 0 0 0-.11.168l-2 5a.5.5 0 0 0 .65.65l5-2a.5.5 0 0 0 .168-.11l.178-.178z"/>
           </svg><a class="btn btn-light" href="{{ url('cotizaciones/'.$cotizacion->id.'/contestar') }}" role="button">
               {{-- <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" class="bi bi-x-circle-fill" viewBox="0 0 16 16">
             <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM5.354 4.646a.5.5 0 1 0-.708.708L7.293 8l-2.647 2.646a.5.5 0 0 0 .708.708L8 8.707l2.646 2.647a.5.5 0 0 0 .708-.708L8.707 8l2.647-2.646a.5.5 0 0 0-.708-.708L8 7.293 5.354 4.646z"/>
           </svg> --}}<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-envelope-fill" viewBox="0 0 16 16">
          <path d="M.05 3.555A2 2 0 0 1 2 2h12a2 2 0 0 1 1.95 1.555L8 8.414.05 3.555ZM0 4.697v7.104l5.803-3.558L0 4.697ZM6.761 8.83l-6.57 4.027A2 2 0 0 0 2 14h12a2 2 0 0 0 1.808-1.144l-6.57-4.027L8 9.586l-1.239-.757Zm3.436-.586L16 11.801V4.697l-5.803 3.546Z"/>
          </svg></a><button type="button" class="btn btn-light" data-bs-toggle="modal" data-bs-target="#exampleModal2"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" class="bi bi-trash-fill" viewBox="0 0 16 16">
              <path d="M2.5 1a1 1 0 0 0-1 1v1a1 1 0 0 0 1 1H3v9a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2V4h.5a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1H10a1 1 0 0 0-1-1H7a1 1 0 0 0-1 1H2.5zm3 4a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 .5-.5zM8 5a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7A.5.5 0 0 1 8 5zm3 .5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 1 0z"/>
            </svg></b></button>
            <!-- Modal -->
          <div class="modal fade" id="exampleModal2" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="D" id="exampleModalLabel"> Eliminar la cotizacion</h5>
                  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                  <div class="alert alert-danger" role="alert">
                  Esta seguro de que desea eliminar la cotizacion
                  </div>
                </div>
                <div class="modal-footer">
                  <form action="{{ url('cotizaciones/'.$cotizacion->id) }}" method="post">
                    @csrf
                      {{ method_field('DELETE') }}
                  <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">No</button>
                  <input type="submit" class="btn btn-primary" value="Si">
                </form>
                </div>
              </div>
            </div>
          </div>
          {{-- <button type="button" class="btn btn-light" data-bs-toggle="modal" data-bs-target="#exampleModal"></button></button> --}}
          <a class="btn btn-light" href="{{ url('cotizaciones/'.$cotizacion->id) }}" role="button"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" class="bi bi-eye-fill" viewBox="0 0 16 16">
             <path d="M10.5 8a2.5 2.5 0 1 1-5 0 2.5 2.5 0 0 1 5 0z"/>
             <path d="M0 8s3-5.5 8-5.5S16 8 16 8s-3 5.5-8 5.5S0 8 0 8zm8 3.5a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7z"/>
           </svg></a></div>

           @endforeach

        </div>
      </div>
      <br>
      <div class="container" style="margin-bottom: 1%">
        <div class="row row col-2" style="margin: 0 auto;"><a href="{{ url('/exportPdfCotizaciones') }}"><button type="button" class="btn btn-lg btn-warning" data-bs-toggle="popover" title="Click para ver el reporte en formato pdf o excel" data-bs-content="And here's some amazing content. It's very engaging.Right?">Reporte PDF</button></a></div>
      </div>
      <script src="{{ asset('js/Eventos/fecha.js') }}"></script>
</body>
</html>
